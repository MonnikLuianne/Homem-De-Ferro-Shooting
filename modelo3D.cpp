...
//proshooting
bool esferaAtiva = false;
bool disparou = false;
float deslocamentoEsfera = 0.0f;
float velocidadeEsfera = 0.05f; //0.15f
...
void init_glut(const char *nome_janela, int argc, char** argv){
    ...
    printf(".");
    animation = glmLoadAnimation("models/ironman/animations/shooting", shooting, 50);
        ...
        if(esferaAtiva)
{
    ...
    // AJUSTAR ESTES VALORES ATÉ FICAR EXATAMENTE NA MÃO
    glTranslatef(-0.25f, 1.55f, deslocamentoEsfera+2.0f);
    ...
    deslocamentoEsfera += velocidadeEsfera;

    if(deslocamentoEsfera > 8.0f)
    {
        esferaAtiva = false;
        disparou = false;
    }
       ...
       if(animation_id == shooting &&
   keyframe == 25 &&
   !disparou)
{
    disparou = true;
    esferaAtiva = true;
    deslocamentoEsfera = 0.0f;
}
